...
describe "the signin process", :js => true do
  ...
  # it "Allows users of type developer to register with the facebook login button" do
  #   visit '/'
  #   expect(page).to have_content 'GET STARTED'    
  #   click_link 'Register'
  #   expect(page).to have_content 'Signing up is easy and fun.'
 ...
 it "Sends an email confirmation when users first register via the form as a developer" do
    visit '/'
    expect(page).to have_content 'GET STARTED'    
    click_link 'Register'
    expect(page).to have_content 'Signing up is easy and fun.'
    ...
    fill_in 'user_first_name', with: "test"
    fill_in 'user_email', with: "test12@mailinator.com"
    fill_in 'user_location', with: "test"
    fill_in 'user_password', with: "12345678"
    fill_in 'user_password_confirmation', with: "12345678"
    click_button "Sign Up"
    expect(page).to have_content 'A message with a confirmation link has been sent to your email address. Please follow the link to activate your account.'
    user = User.find_by_email("test12@mailinator.com") 
    user.should_not be_nil
    user.confirmation_token.should_not be_nil
    token = ActionMailer::Base.deliveries.last.body.match(/confirmation_token=(.+)">/x)[1]
 ...
 it "Sends an email confirmation when users first register via the form as a employer" do
    visit '/'
    expect(page).to have_content 'GET STARTED'    
    click_link 'Register'
    expect(page).to have_content 'Signing up is easy and fun.'
    page.find("#type_Employer").click
    fill_in 'user_first_name', with: "test"
    fill_in 'user_email', with: "test13@mailinator.com"
    fill_in 'user_location', with: "test"
    fill_in 'user_password', with: "12345678"
    fill_in 'user_password_confirmation', with: "12345678"
    click_button "Sign Up"
    expect(page).to have_content 'A message with a confirmation link has been sent to your email address. Please follow the link to activate your account.'
    user = User.find_by_email("test13@mailinator.com") 
    user.should_not be_nil
    user.confirmation_token.should_not be_nil
    token = ActionMailer::Base.deliveries.last.body.match(/confirmation_token=(.+)">/x)[1]
    visit "/users/confirmation?confirmation_token=#{token}"
